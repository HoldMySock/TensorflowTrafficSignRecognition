...
def show_classify_button(file_path):
    classify_b = Button(top, text="Розпізнати знак", command=lambda: classify(file_path), padx=10, pady=5)
...
def upload_image():
    try:
        ...
        # converted_img = Image.open(r'C:\Users\Slavik\AppData\Local\Temp\temp_converted_image.png')
        uploaded.thumbnail(((top.winfo_width() / 2.25), (top.winfo_height() / 2.25)))
        im = ImageTk.PhotoImage(uploaded)
        sign_image.configure(image=im)
        sign_image.image = im
        label.configure(text='')
        show_classify_button(r'D:\pycharmproj\TensorflowTrafficSignRecognition\temp\temp_converted_image.png')
    except:
        pass


upload = Button(top, text="Завантажити знак", command=upload_image, padx=10, pady=5)
upload.configure(background='#364196', foreground='white', font=('arial', 10, 'bold'))

upload.pack(side=BOTTOM, pady=70)
sign_image.pack(side=BOTTOM, expand=True)
label.pack(side=BOTTOM, expand=True)
heading = Label(top, text="Розпізнати знак", pady=40, font=('Times New Roman', 25, 'bold'))
